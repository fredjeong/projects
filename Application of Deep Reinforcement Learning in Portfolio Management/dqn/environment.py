...
class TradingEnv:
    ...
    def __init__(self, df, initial_balance=1000, lookback_window_size=50, render_range = 100):
        ...
        self.Target = copy.deepcopy(self.Behaviour).to(device)
...
def train_agent(env, visualize=False, train_episodes=1, training_batch_size=500):
    ...
    for episode in range(train_episodes):
        ...
        np.savetxt(f'./dqn/records/benchmark_{episode}.csv', env.benchmark)
        np.savetxt(f'./dqn/records/portfolio_values_{episode}.csv', env.portfolio_values)
        if episode == train_episodes - 1:
            env.save()
    np.savetxt('./dqn/records/losses.csv', env.losses)
    ...
    for episode in range(test_episodes):
        ...
        while True:
            ...
            if env.current_step == env.end_step:
                break
    np.savetxt('./dqn/records/portfolio_values.csv', env.portfolio_values)