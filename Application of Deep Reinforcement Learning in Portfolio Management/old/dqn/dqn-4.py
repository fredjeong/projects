...
class BitcoinTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 0:  # 구매
            buy_amount = min(self.fiat, self.fiat * 0.1)
            self.asset += buy_amount * (1 - self.transaction_cost) / current_price
            self.fiat -= buy_amount
        elif action == 1:  # 매도
            sell_amount = min(self.asset, self.asset * 0.1)
        ...
        print(f"Step: {self.current_step}, Action: {action}, Reward: {reward}, Value: {current_value}")  # 로깅 추가
...
model.learn(total_timesteps=10000)  # 학습 횟수 증가
